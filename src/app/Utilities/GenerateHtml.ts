export class GenerateHtml
{
  Create(firstName: string, lastName: string, nameCustomer: string, laDate: Date )
  {
    return  '<h1 style="text-align: center;">Rapport de ' +firstName+' '+lastName+'</h1><p style="padding-left:2vw;"><b><u>Client</u>: '+nameCustomer+'</b><p style="padding-left:2vw;"><b><u>Nr° de carte ministérielle</u></b> : </p><br><p style="padding-left:2vw;"><b>'+ laDate +'</b></p><br>'+`
    <br>
      <table style="width:100%;">
        <thead>
          <tr style="display: table-row;">
            <th style="border-bottom:1px black solid; border-bottom:1px black solid; width: 15%; background-color:#326290;color:#F5DEB3;">Heure de début</th>
            <th style="border-bottom:1px black solid; border-bottom:1px black solid; width: 15%; background-color:#326290;color:#F5DEB3;">Heure de fin</th>
            <th style="border-bottom:1px black solid; width:70%;height:30px; background-color: #326290; color:#F5DEB3;">Message</th>
          </tr>
        </thead>
        <tbody>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"><br></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid; vertical-align: top; height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>
          <tr style="display: table-row;" >
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
            <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
            <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
          </tr>

        </tbody>
      </table>
    `
  }

  AddLines()
  {
    const table = document.querySelector('table');
    const tbody = table.querySelector('tbody');
    const newRow = `<tr style="display: table-row;" >
      <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
      <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
      <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
    </tr>
    <tr style="display: table-row;" >
      <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
      <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
      <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
    </tr>
    <tr style="display: table-row;" >
      <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
      <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
      <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
    </tr>
    <tr style="display: table-row;" >
      <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
      <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
      <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
    </tr>
    <tr style="display: table-row;" >
      <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
      <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
      <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
    </tr>
    <tr style="display: table-row;" >
      <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
      <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
      <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
    </tr>
    <tr style="display: table-row;" >
      <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all;"></td>
      <td style="border-bottom:1px black solid; vertical-align: top; text-align:center; word-break: break-all; "></td>
      <td style="border-bottom:1px black solid;  height: 20px; word-break: break-all;"></td>
    </tr>
  `
  return tbody.insertAdjacentHTML('beforeend', newRow);
  }
}
